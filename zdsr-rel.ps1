# PowerShell 脚本：自动打包发布 nvda2zdsr
# 功能：复制文件到指定目录，创建带时间戳的文件夹并压缩为zip

param(
    [string]$BaseDir = "nvda2zdsr",
    [switch]$Force = $false
)
...
Write-Host "=== nvda2zdsr 自动打包发布脚本 ===" -ForegroundColor Green
...
try {
    ...
    $x64Files = @{
        ...
        "VS2022\x64\ZDSR-Rel\ZDSRAPI_x64.dll" = "ZDSRAPI_x64.dll"
